...
export function PreviewModal({ isOpen, onClose, itemId, rootId, relPath, title, path, type, size }: PreviewModalProps) {
  ...
  async function openWindows() {
    ...
    try {
      ...
      setOpenInfo(result.openedWith === 'quickviewer' ? '已使用 QuickViewer 打开。' : '已调用 Windows 默认程序打开。');
    } catch (error) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/55 p-3 sm:p-6" onClick={onClose}>
      ...
      >
        ...
        <div className="flex-1 overflow-hidden p-4">
          {!itemId ? <Empty text="该文件尚未建立索引，无法在浏览器内预览。你仍可以点击“在 Windows 中打开”。" /> : null}
          {itemId && preview.isLoading ? <Empty text="正在加载预览..." /> : null}
          {itemId && preview.tooLarge ? <Empty text="文件过大，建议直接在 Windows 中打开。" /> : null}
